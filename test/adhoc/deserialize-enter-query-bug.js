...
function getLocalEid(world, eid) {
  const $localEntities = Object.getOwnPropertySymbols(world)[17];
  // @ts-ignore
...
describe("adhoc deserialize enter query bug", () => {
  ...
  it("should", () => {
    ...
    assert.equal(enter(world2)[0], lid, "World 2 Enter should be 0");
    assert.equal(Component2.x[lid], x, "Should have x value");
    assert.deepEqual(
      Array.from(Component2.y[lid]),
      [1, 2, 3, 4],
      "Should have y value"
    );
    assert.equal(
      Component2.rotation[lid],
      rotation,
      "Should have rotation value"
    );
  });
});

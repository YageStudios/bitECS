...
export const canonicalize = (target) => {
  ...
  const allChangedProps = [...changedComponentProps, ...changedProps].reduce(
    (map, prop) => {
      const $ = Symbol();
      createShadow(prop, $);
      map.set(prop, $);
 ...
 */
export const defineSerializer = (target, world, maxBytes = 20000000) => {
      ...
      for (let i = 0; i < ents.length; i++) {
        ...
        const newlyAddedComponent =
          ...
          (!componentCache.has(component) &&
            ...
            hasComponent(reqWorld, component, eid));
    ...
    while (where < packet.byteLength) {
      ...
      for (let i = 0; i < entityCount; i++) {
        ...
        if (ArrayBuffer.isView(prop[eid])) {
          ...
          const value =
            view[`get${prop.constructor.name.replace("Array", "")}`](where);
          where += prop.BYTES_PER_ELEMENT;

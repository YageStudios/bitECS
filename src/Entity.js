...
export const $globalEntityCursor = Symbol("globalEntityCursor");
export const $globalSize = Symbol("globalSize");
...
export const getEntityCursor = (world) => world[$globalEntityCursor];
...
export const addEntity = (world) => {
  const eid = world[$manualEntityRecycling]
    ? world[$removed].length
      ? world[$removed].shift()
      : world[$globalEntityCursor]++
    : world[$removed].length >
      Math.round(world[$globalSize] * removedReuseThreshold)
    ? world[$removed].shift()
    : world[$globalEntityCursor]++;
...
export const removeEntity = (world, eid) => {
  ...
  // Free the entity
  if (world[$manualEntityRecycling]) world[$recycled].push(eid);
  else world[$removed].push(eid);
